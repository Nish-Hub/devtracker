...
| OrchestratorLLM smart routing | ✅ Done | Vendored router core (`js/orchestrator/`); provider capability tags + smart-routing toggle in AI settings; AI calls routed by task Big O class through a verify+escalate cascade with a hard privacy guard; routing logged (model/class/escalations/tokens/cost) to the Home timeline. Built from the standalone OrchestratorLLM project (`~/Documents/OrchestratorLLM`). Local Ollama models (`qwen2.5-coder:3b`, `llama3.2`) are preset providers. |
| Playground (journey simulator) | ✅ Done | Per-project "Playground" tab: define journeys as ordered steps (mock API request/response, Postman-style, or mock UI screens rendered in a sandboxed iframe); step through with Prev/Next/Run/Reset and a live journey map; optional ✨ Generate-mock via the local model. Journeys persist in the store (`project.journeys`). |
| Coder (agentic, local models) | ✅ Done | Per-project "Coder" tab (desktop only): chat wired to a local/cloud model, grounded in the project briefing + constraints + **live file tree**. ReAct tool loop — read_file/list_dir run freely; write_file shows a diff you Apply (with **↩ Undo**); run_command shows the command you Run. Two-column layout with a **file sidebar** (click to attach files to context); the real file list is injected into the system prompt so the model uses real paths (fixed the `src/` hallucination). Streaming with a live cursor, **Stop** button, and a 60s stall watchdog. All scoped to a picked workdir with a path-traversal guard in `main.js`. Companion: Aider (`~/Documents/aider-local.sh`). |
