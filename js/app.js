...
function openInDrawio(){const project=activeProject();if(!project.architecture || !project.architecture.content){toast('No architecture to open.');return;}if(project.architecture.type==='svg' || project.architecture.name.toLowerCase().endsWith('.svg')){ // open SVG in new tab
  const w=window.open();w.document.write(project.architecture.content);return;}
// otherwise provide a download and open instructions or attempt to open diagrams.net
