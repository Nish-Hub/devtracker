...
function renderArchitecture(){const el=$('#architectureView');if(!el)return;el.innerHTML=`<div class="view-head"><div><p class="eyebrow">ARCHITECTURE PATTERNS</p><h1>Architecture options</h1><p class="subcopy">Choose from common software architecture styles and compare how they fit the project.</p></div></div><div class="list-card architecture-grid">${ARCHITECTURES.map(a=>`<article class="list-row architecture-card"><strong>${esc(a.name)}</strong><p>${esc(a.summary)}</p><details><summary>Details</summary><p>${esc(a.details)}</p></details></article>`).join('')}</div>`;}
